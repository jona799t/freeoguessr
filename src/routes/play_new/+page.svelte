<script>
    ...
    const map = JSON.parse(localStorage.getItem("maps"))[map_id];
    console.log(map)
    ...
    async function guess(marker) {
        ...
        message = `You were ${distance(marker._latlng.lat, marker._latlng.lng, map.data[round].lat, map.data[round].lng).toFixed(2)} meters away from the target`
        blur = true;
        
        await new Promise(resolve => setTimeout(resolve, 2000));
        if (round < map.data.length-1 && round < max_round-1) {
            round++;
            loaded();
        } else {
...
{#if map}
    ...
    <div class="absolute top-0 w-full {blur ? "blur-xl" : ""} -z-10">
        <iframe
            ...
            src="https://www.google.com/maps/embed?pb=!6m8!1m7!{map.data[round].panoId}!2m2!1d{map.data[round].lat}!2d{map.data[round].lng}!3f{map.data[round].heading}!4f{map.data[round].pitch}!5f{map.data[round].zoom}">
        </iframe>
    </div>
{:else}
